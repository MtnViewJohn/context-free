 ...
 */
- (void) splitView:(DMSplitView *)splitView divider:(NSUInteger) dividerIndex movedAt:(CGFloat) newPosition;
 ...
 */
- (BOOL) setPosition:(CGFloat)position ofDividerAtIndex:(NSUInteger)dividerIndex animated:(BOOL) animated
...
/** Set the new size of given subview at index. A proportional shrink/grow is applied to involved dividers (left/right, left or right)
 ...
 @param     completition            completition block handler
 */
- (BOOL) setSize:(CGFloat) size ofSubviewAtIndex:(NSUInteger) subviewIndex animated:(BOOL) animated
