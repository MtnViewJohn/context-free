    ...
    template<class ColorT, class Order> struct multiplier_rgba
    {
        ...
        static AGG_INLINE void premultiply(value_type* p)
        {
            ...
            if(a < ColorT::base_mask)
            {
                ...
                p[Order::R] = value_type(ColorT::int_mult(p[Order::R], a));
                p[Order::G] = value_type(ColorT::int_mult(p[Order::G], a));
                p[Order::B] = value_type(ColorT::int_mult(p[Order::B], a));
            }
        }
    ...
    template<class Blender> struct copy_or_blend_rgba_wrapper
    {
                                                 ...
                                                 unsigned cover)
        {
            ...
            else
            {
                ...
                {
                    //alpha = color_type::int_mult_cover(alpha, cover)
                    if(alpha == base_mask && cover == cover_mask)
                    {
                        p[order_type::R] = cr;
                        p[order_type::G] = cg;
                        p[order_type::B] = cb;
